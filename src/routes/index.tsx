...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Imported French perfumery, curated by emotion. Discover your scent through THE AURA's Scent Journey." },
    ],
  }),
});

// Hardware-accelerated 3D tilt — desktop only, minimal re-renders
function useBottleTilt() {
  ...
  const bind = () => {
    ...
    return {
      onUnmount: () => window.removeEventListener("mousemove", handler)
    };
  };

  return { rotateX, rotateY, translateX, translateY, bind };
}

// Group perfumes by aura/brand family
const collections = [
  {
    id: "house-aura",
    name: "House AURA",
...
function Index() {
  ...
  const { rotateX, rotateY, translateX, translateY, bind } = useBottleTilt();
  const tiltHandlers = bind();

  // Floating note positions — pure CSS animation, no JS overhead
  const floatingNotes = [
  ...
  return (
    <>
      {/* CSS for hardware-accelerated floating notes */}
      <style>{`
      ...
      {/* HERO — Living Fragrance Gallery */}
      <section 
        ref={heroRef}
        className="theme-noir bg-background text-foreground relative overflow-hidden"
      >
        {/* Atmospheric layers — static, no animation */}
        <div className="absolute inset-0 grain opacity-25 pointer-events-none z-20" />
        ...
        {/* Vignette for depth */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_transparent_0%,_rgba(0,0,0,0.15)_100%)] pointer-events-none z-10" />

        {/* Main stage */}
        <div className="relative z-10 min-h-[100dvh] flex flex-col">
          <div className="flex-1 flex items-center">
            <div className="w-full max-w-[1400px] mx-auto px-6 lg:px-16 py-16 lg:py-0">
              <div className="grid lg:grid-cols-12 gap-8 lg:gap-6 items-center min-h-[60vh] lg:min-h-[80vh]">
                
                {/* LEFT: Editorial typography */}
                <div className="lg:col-span-5 relative z-30 order-2 lg:order-1 pt-4 lg:pt-0">
                  {/* Gallery label — desktop vertical */}
                  <motion.div
                  ...
                  <motion.p
                    ...
                    className="lg:hidden text-[10px] uppercase tracking-luxe text-muted-foreground mb-4"
                  >
                  ...
                  <motion.p
                    ...
                    className="mt-6 lg:mt-8 text-sm lg:text-[15px] text-muted-foreground max-w-xs leading-relaxed"
                  >
                  ...
                  >
                    ...
                    >
                      <span className="relative z-10 transition-colors duration-500 group-hover:text-background">Explore Collection</span>
                ...
                {/* RIGHT: Bottle display */}
                <div className="lg:col-span-7 relative order-1 lg:order-2 h-[38vh] sm:h-[48vh] lg:h-[78vh]">
                  {/* Aura — single instance, optimized */}
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <Aura variant="silver" intense className="!opacity-15 scale-110" />
                  </div>

                  {/* Floating scent notes — CSS-only, zero JS overhead */}
                  ...
                  {/* Bottle with 3D parallax — GPU only */}
                  <motion.div
                  ...
                  >
                    <motion.div
                      ...
                      className="relative h-full w-full max-w-[280px] sm:max-w-sm lg:max-w-md xl:max-w-lg mx-auto"
                    >
                      <img
                        src={heroImg}
            ...
            <div className="animate-marquee whitespace-nowrap flex">
              {[...Array(4)].map((_, i) => (
                <div key={i} className="flex items-center gap-8 lg:gap-12 px-4 lg:px-6">
                  {["Composed in Grasse", "Single-origin essences", "Complimentary global shipping", "Concierge gifting", "Established with intention"].map((text) => (
                    <span key={text} className="flex items-center gap-3 text-[9px] lg:text-[10px] uppercase tracking-luxe text-muted-foreground/50 shrink-0">
      ...
      {/* IMMEDIATE PRODUCT DISCOVERY — Signature merged into hero flow */}
      <section id="collection" className="bg-background relative">
        {/* Section header */}
        <div className="mx-auto max-w-[1400px] px-6 lg:px-16 pt-20 lg:pt-28 pb-12">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <h2 className="font-display text-4xl lg:text-5xl mt-3 leading-tight">
                Filter by feeling,<br className="hidden lg:block" /> not by note.
              </h2>
            </div>
        ...
        {/* Horizontal product strip — immediately shoppable */}
        <div className="relative overflow-hidden">
          <div className="mx-auto max-w-[1400px] px-6 lg:px-16">
            <div className="flex gap-4 lg:gap-6 overflow-x-auto pb-8 snap-x snap-mandatory scrollbar-hide -mx-6 px-6 lg:mx-0 lg:px-0">
              {perfumes.slice(0, 5).map((perfume, i) => (
                <motion.div
                  ...
                  className="snap-start shrink-0 w-[260px] lg:w-[300px] group"
                >
                  <Link to={`/shop/${perfume.id}`} className="block">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex gap-1.5 pt-1">
                        {perfume.mood.slice(0, 2).map(m => (
                          <span key={m} className="text-[9px] uppercase tracking-luxe text-muted-foreground/60 border border-border/50 px-2 py-0.5">
                            {m}
                          </span>
                        ))}
                      </div>
        ...
        {/* Mood filter grid */}
        <div className="mx-auto max-w-[1400px] px-6 lg:px-16 py-12 border-t border-border/10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-border">
            {[
              ...
              <div key={item.k} className="bg-background p-5 lg:p-6 group hover:bg-secondary/30 transition-colors duration-500 cursor-pointer">
      ...
      {/* COLLECTIONS — Grouped by brand/aura, 10+ items */}
      ...
      <section className="theme-noir bg-background text-foreground py-32 relative overflow-hidden">
        ...
        <div className="mx-auto max-w-5xl px-6 lg:px-10 text-center relative z-10">
          ...
          <h2 className="font-display text-5xl md:text-7xl mt-6 leading-tight">
            Every fragrance has<br />
            <em className="italic">a visible soul.</em>
          </h2>
          <p className="mt-8 text-lg text-muted-foreground max-w-2xl mx-auto">
            We render each composition as a moving aura — generated from its
            notes. Warm oud breathes in golden ember; cool vetiver drifts in
            pale silver. Scent, finally, made seeable.
          </p>
        </div>
      </section>
    </>
...
// Collection block component — lazy, intersection-observer triggered
function CollectionBlock({ collection, index }: { collection: typeof collections[0]; index: number }) {
  ...
  return (
    <div ref={ref} className="relative">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className={`w-2 h-2 rounded-full ${
              index === 0 ? "bg-amber-500/60" : 
              index === 1 ? "bg-rose-400/60" : 
              index === 2 ? "bg-slate-300/60" : "bg-cyan-400/60"
            }`} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        ...
        {/* Fill remaining slots with placeholders for visual balance if needed */}
        {collection.perfumes.length < 4 && [...Array(4 - collection.perfumes.length)].map((_, i) => (
          <div key={`placeholder-${i}`} className="aspect-[3/4] bg-secondary/30 rounded-sm flex items-center justify-center border border-border/20 border-dashed">
